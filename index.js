...
io.on('connection', (socket) => {
    ...
    socket.on('slaveSendAlert', data => {
        ...
        logins.findOne({ '_id': data['projectId'] })
            .then(doc => {
                if (doc) {
                    doc['fcmTokens'].forEach(token => {
                        sendPushNotification(data, token[0]);
                    })
                }
            }).catch(e => {
                console.log('====== error ====', e);
    ...
    socket.on('addFcmToken', data => {
        console.log(data);
        db.saveFcmToken(data).then(res => {
            if (res) {
                console.log('token insert sucessfully');
            }
        }).catch(e => {
            console.log(e);
        });
    ...
    socket.on('removeFcmToken', data => {
        // console.log('===== remove token =====' , data);
        db.removeFcmToken(data).then(res => {
            if (res) {
                console.log('token remove sucessfully');
            }
        }).catch(e => {
            console.log(e);
        });
...
const sendPushNotification = (data, token) => {
